resource "sumologic_dashboard" "auth___connections_and_clients" {
  ...
  panel {
    sumo_search_panel {
      ...
      query {
        query_key    = "A"
        query_string = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"client_name\", \"type\" | where client_name matches \"*\" and type = \"s\" | timeslice by 1d\n|where if (\"{{client_name}}\" = \"*\", true, client_name matches \"{{client_name}}\")\n|count by _timeslice, client_name | transpose row _timeslice column client_name"
        query_type   = "Logs"
      }
  ...
  panel {
    sumo_search_panel {
      ...
      query {
        query_key    = "A"
        query_string = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"client_name\", \"ip\", \"type\"\n| where type = \"s\"\n| lookup country_name from geo://location on ip=ip\n|where if (\"{{client_name}}\" = \"*\", true, client_name matches \"{{client_name}}\") AND if (\"{{country_name}}\" = \"*\", true, country_name matches \"{{country_name}}\")\n|count by client_name, country_name\n| transpose row client_name column country_name"
        query_type   = "Logs"
      }
  ...
  panel {
    sumo_search_panel {
      ...
      query {
        query_key    = "A"
        query_string = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"client_name\" | where !isEmpty(client_name)\n|where if (\"{{client_name}}\" = \"*\", true, client_name matches \"{{client_name}}\")\n|count client_name | top 10 client_name by _count"
        query_type   = "Logs"
      }
  ...
  variable {
    ...
    source_definition {
      log_query_variable_source_definition {
        field = "client_name"
        query = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"client_name\", \"ip\", \"type\"\n| where type = \"s\"\n| lookup country_name from geo://location on ip=ip"
      }
    }
  }
  ...
  variable {
    ...
    source_definition {
      log_query_variable_source_definition {
        field = "connection"
        query = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"connection\" | where connection matches \"*\" | timeslice by 1h"
      }
    }
  }
...
resource "sumologic_dashboard" "auth___overview" {
  ...
  panel {
    sumo_search_panel {
      ...
      query {
        query_key    = "A"
        query_string = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"type\", \"ip\" | where type in (\"fp\", \"fu\") | where !isEmpty(ip)\n|where if (\"{{ip}}\" = \"*\", true, ip matches \"{{ip}}\")\n|count ip | top 10 ip by _count"
        query_type   = "Logs"
      }
  ...
  panel {
    sumo_search_panel {
      ...
      query {
        query_key    = "A"
        query_string = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"type\", \"user_name\" | where type in (\"fp\", \"fu\") | where !isEmpty(user_name)\n|where if (\"{{user_name}}\" = \"*\", true, user_name matches \"{{user_name}}\")\n|count user_name | top 10 user_name by _count"
        query_type   = "Logs"
      }
  ...
  panel {
    sumo_search_panel {
      ...
      query {
        query_key    = "A"
        query_string = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"type\" | where type matches(\"gd_*\") | timeslice by 1h\n|where if (\"{{type}}\" = \"*\", true, type matches \"{{type}}\")\n|count by _timeslice, type | transpose row _timeslice column type"
        query_type   = "Logs"
      }
  ...
  panel {
    sumo_search_panel {
      ...
      query {
        query_key    = "A"
        query_string = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"type\", \"user_name\" | where type = \"s\" | where !isEmpty(user_name)\n|where if (\"{{user_name}}\" = \"*\", true, user_name matches \"{{user_name}}\")\n|count user_name | top 10 user_name by _count"
        query_type   = "Logs"
      }
  ...
  panel {
    sumo_search_panel {
      ...
      query {
        query_key    = "A"
        query_string = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"user_agent\" | parse field=user_agent \"* \" as user_agent\n|where if (\"{{user_agent}}\" = \"*\", true, user_agent matches \"{{user_agent}}\")\n|count user_agent | top 10 user_agent by _count"
        query_type   = "Logs"
      }
  ...
  panel {
    sumo_search_panel {
      ...
      query {
        query_key    = "A"
        query_string = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"user_name\", \"user_agent\", \"client_name\" | parse field=user_agent \"/ * \" as user_os\n|where if (\"{{user_os}}\" = \"*\", true, user_os matches \"{{user_os}}\")\n|count_distinct(user_name) group by user_os | top 10 user_os by _count_distinct"
        query_type   = "Logs"
      }
  ...
  variable {
    ...
    source_definition {
      log_query_variable_source_definition {
        field = "country_code"
        query = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"ip\", \"type\"\n| where type = \"s\"\n| lookup latitude, longitude, country_code, country_name, region, city from geo://location on ip = ip"
      }
    }
  }
  ...
  variable {
    ...
    source_definition {
      log_query_variable_source_definition {
        field = "ip"
        query = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"type\", \"ip\" | where type in (\"fp\", \"fu\") | where !isEmpty(ip)"
      }
    }
  }
  ...
  variable {
    ...
    source_definition {
      log_query_variable_source_definition {
        field = "type"
        query = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"type\" | where type matches(\"gd_*\") | timeslice by 1h"
      }
    }
  }
  ...
  variable {
    ...
    source_definition {
      log_query_variable_source_definition {
        field = "user_agent"
        query = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"user_agent\" | parse field=user_agent \"* \" as user_agent"
      }
    }
  }
  ...
  variable {
    ...
    source_definition {
      log_query_variable_source_definition {
        field = "user_name"
        query = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"type\", \"user_name\" | where type in (\"fp\", \"fu\") | where !isEmpty(user_name)"
      }
    }
  }
  ...
  variable {
    ...
    source_definition {
      log_query_variable_source_definition {
        field = "user_os"
        query = "${var.scope_key}={{${var.scope_key_variable_display_name}}}  | json \"user_name\", \"user_agent\", \"client_name\" | parse field=user_agent \"/ * \" as user_os"
      }
    }
  }
}
